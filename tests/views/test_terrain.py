...
class TerrainTest(unittest.TestCase):
    def test_AjoutAffichageObjet(self):
        ...
        for _ in range(0, 10):
            ...
            cpt = cpt + 1
        self.assertTrue(sum((el is not None for ligne in t.grille for el in ligne)) == cpt)
    ...
    def test_Grille(self):
        for _ in range(100):
            ...
            self.assertTrue(all(len(line) == random_colonne for line in grille))
    ...
    def test_AjoutNAlea(self):
        for _ in range(50):
            ...
            self.assertTrue(sum((el is not None for ligne in t.grille for el in ligne)) == random_colonne)

        t = Terrain.Terrain(1, 1)
        t.ajout_alea(3)
        self.assertTrue(sum((el is not None for ligne in t.grille for el in ligne)) == 0)
    ...
    def test_estVide(self):
        ...
        self.assertTrue(any([T0.casevide(x, x) for x in range(-1, 2)]) == False)
        ...
        self.assertTrue(any([T1.casevide(x, y) for x in range(-1, 2) for y in range(-1, 2)]) == False)
            ...
            for l in range(0, T.nbLignes):
                for c in range(0, T.nbColonnes):
                    if l == aleaX and c == aleaY:
                        self.assertTrue(T.casevide(l, c) == False)  # Test sur l'unique case non vide du tableau
                    else:
                        self.assertTrue(T.casevide(l, c) == True)  # Test sur l'ensemble des cases vide du tableau
    ...
    def test_supprimerObjet(self):
        ...
        # Cas de terrain de taille zero => test HORS tableau dans ce cas précis
        T0 = Terrain.Terrain(0, 0)
        self.assertTrue(any([T0.supprimerObjet(x, x) for x in range(-1, 2)]) == False)

        # Cas de terrain de taille une 
        T1 = Terrain.Terrain(1, 1)
        # Test de la fonction sur les cases hors de la grille
        self.assertTrue(any([T1.supprimerObjet(x, y) for x in range(-1, 2) for y in range(-1, 2) if x != y]) == False)
        self.assertTrue(T1.supprimerObjet(0, 0) == True)  # test sur la case VIDE du tableau
        T1.ajout_objet(o, 0, 0)
        self.assertTrue(T1.supprimerObjet(0, 0))  # test sur case pleine du tableau
        ...
        for _ in range(1000):
            ...
            self.assertTrue(all([T.supprimerObjet(x, y) for x in range(0, T.nbLignes) for y in range(0, T.nbColonnes)]))
            # Test toutes cases vide aprés passage fonction
            self.assertTrue(all([T.casevide(x, y) for x in range(0, T.nbLignes) for y in range(0, T.nbColonnes)]))
